...
## __init__ c'est le constructeur qui 
...
## le passage "forward" du réseau de neurones.


# Structure du modèle
class BigramLanguageModel(nn.Module):
    def __init__(self, n_embd, block_size, n_head, n_layer, dropout):
        super().__init__()
...
class Head(nn.Module):
    def __init__(self, head_size, n_embd, block_size, dropout):
        ...
        self.dropout = nn.Dropout(dropout) #Le dropout dont nous discutons dans le livre
...
# Entraîne le modèle
def train(model, optimizer, max_iters, eval_iters, eval_interval, block_size, batch_size):
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train and/or infer with a language model")
    ...
    parser.add_argument("--save_model", type=str, help="Sauvegarde le modèle dans le fichier spécifié")
    parser.add_argument("--load_model", type=str, help="Charge le modèle depuis le fichier spécifié")

    # Paramètres du modèle
    parser.add_argument("--batch_size", type=int, default=16, help="Nombre d'I/O que le modèle doit apprendre par batch")
    parser.add_argument("--block_size", type=int, default=32, help="Longueur des séquences que le transformer doit apprendre")
    parser.add_argument("--max_iters", type=int, default=1000, help="Nombre d'itérations d'apprentissage")
    parser.add_argument("--eval_interval", type=int, default=300, help="Intervalle d'évaluation pendant l'entraînement")
    parser.add_argument("--learning_rate", type=float, default=1e-3, help="Taux d'apprentissage")
    parser.add_argument("--eval_iters", type=int, default=200, help="Nombre d'itérations d'évaluation")
    parser.add_argument("--n_embd", type=int, default=32, help="Dimension de l'espace dans lequel on projette les caractères")
    parser.add_argument("--n_head", type=int, default=8, help="Nombre de têtes d'attention")
    parser.add_argument("--n_layer", type=int, default=6, help="Nombre de couches")
    parser.add_argument("--dropout", type=float, default=0.2, help="Probabilité de dropout")
    parser.add_argument("--head_size", type=int, default=16, help="Dimension des têtes d'attention dans laquelle sont projettés les caractères")
...
def main():
    ...
    if args.train:
        ...
        model = BigramLanguageModel(n_embd, block_size, n_head, n_layer, dropout)
        m = model.to(device)
        optimizer = torch.optim.AdamW(m.parameters(), lr=learning_rate)
        train(m, optimizer, max_iters, eval_iters, eval_interval, block_size, batch_size)
    ...
    # Mode inférence
        ...
        if args.load_model:
            # Charge le modèle
            model = load_model(BigramLanguageModel(n_embd, block_size, n_head, n_layer, dropout), args.load_model)
            m = model.to(device)
            ...
            print("Aucun modèle spécifié pour l'inférence. Veuillez spécifier --load_model.")
